...
// Monocart HTML report: written to ./charlie-monocart/ inside the project dir,
// then uploaded to R2 file-by-file so the Worker can serve it at
// GET /api/runs/:id/report (relative asset links keep working).
const MONOCART_DIR = 'charlie-monocart'
const MONOCART_SHIM = 'charlie-monocart.cjs'
const MAX_REPORT_FILES = 200

// A tiny reporter shim so we control monocart's options without touching the
// repo's playwright.config (CLI --reporter can't pass options).
const MONOCART_SHIM_SOURCE = `// Generated by Charlie. Wraps monocart-reporter so the HTML report always
// lands in ./${MONOCART_DIR}/report.html regardless of the repo's own config.
const mod = require('monocart-reporter')
const MonocartReporter = mod.default || mod
module.exports = class CharlieMonocartReporter extends MonocartReporter {
  constructor() {
    super({
      name: process.env.CHARLIE_REPORT_NAME || 'Charlie run',
      outputFile: './${MONOCART_DIR}/report.html',
    })
  }
}
`
...
/**
 * Make the monocart reporter available: install the package (--no-save keeps
 * the repo's package.json/lockfile untouched) and write the option shim.
 * Returns the --reporter argument, or null when unavailable (best-effort —
 * a missing HTML report must never fail the test run).
 */
function ensureMonocart(projectDir: string, env: Env, log: string[]): string | null {
  try {
    if (!existsSync(join(projectDir, 'node_modules', 'monocart-reporter'))) {
      const res = exec(
        ['npm', 'install', '--no-save', '--no-audit', '--no-fund', 'monocart-reporter'],
        { cwd: projectDir, env },
      )
      if (res.code !== 0) {
        log.push(`  (monocart-reporter install exited ${res.code}; skipping HTML report)`)
        return null
      }
    }
    writeFileSync(join(projectDir, MONOCART_SHIM), MONOCART_SHIM_SOURCE)
    return `./${MONOCART_SHIM}`
  } catch (err) {
    log.push(`  (monocart setup failed: ${err instanceof Error ? err.message : String(err)})`)
    return null
  }
}
...
export async function runCodeFlow(opts: {
  ...
  try {
    ...
    // Checkouts are reused across a shard's flows — drop the previous flow's
    // monocart output so it isn't re-uploaded under this flow's prefix.
    rmSync(join(projectDir, MONOCART_DIR), { recursive: true, force: true })
    let command: string[]
    if (spec.testCommand) {
      // A custom command controls its own reporters; we still pick up a
      // ./charlie-monocart output dir below if the command produced one.
      command = ['sh', '-c', spec.testCommand]
    } else {
      const monocart = ensureMonocart(projectDir, env, log)
      command = [
        'npx',
        'playwright',
        'test',
        `--reporter=list,json${monocart ? `,${monocart}` : ''}`,
      ]
    ...
    // Upload the monocart HTML report — a self-contained report.html plus any
    // attachments monocart copied alongside — preserving relative paths so the
    // Worker can serve the whole directory under /api/runs/:id/report/*.
    const monocartDir = join(projectDir, MONOCART_DIR)
    if (existsSync(monocartDir)) {
      const files = walk(monocartDir, () => true)
      let uploaded = 0
      for (const file of files) {
        if (uploaded >= MAX_REPORT_FILES) {
          log.push(`  (monocart upload capped at ${MAX_REPORT_FILES} files)`)
          break
        }
        const rel = file.slice(monocartDir.length + 1)
        try {
          artifactKeys.push(
            await upload(`monocart/${rel}`, await readFile(file), contentTypeFor(rel)),
          )
          uploaded++
        } catch {
          // best-effort — skip files whose names the presign rejects, etc.
        }
      }
      if (uploaded > 0) log.push(`  uploaded monocart report (${uploaded} files)`)
    }
